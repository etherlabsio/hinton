...
##the data related hyperparatemers, can it separete in ethertext.json, for easy tweaking

class Config(NamedTuple):
    """ Config for classification dataset """
    task: str = "infer"
    vocab_file: str = "../uncased_L-12_H-768_A-12/vocab.txt"
    data_file: dict = {"train": "data/train.csv",
                       "eval": "data/test.csv"}
    max_len: int = 128
...
def get_class(task):
    ...
    table = {"ether": EtherText, "agnews": AGNews, "infer": Infer}
    return table[task]
...
class EtherText(PreprocessedTextDataset):
...
class Infer(PreprocessedTextDataset):
    """ Dataset class for MNLI """
    labels = ("0", "1") # label names
    def __init__(self, *args, **kwags):
        super().__init__(*args, **kwags)
    ...
    def get_instances(self, instance):
        labels, sent_1,sent_2 = instance
        yield labels,sent_1,sent_2
...
if __name__ == '__main__':
    ...
    cfg = Config(task="ether",
                 vocab_file="uncased_L-12_H-768_A-12/vocab.txt",
                 data_file={"train": "/home/shivam/Desktop/bert_distillation/data/ether_meetings/train_nsp.csv",
                            "eval": "/home/shivam/Desktop/bert_distillation/data/ether_meetings/val_nsp.csv"},
                 max_len=16)
    ...
    for i in range(len(pipelines)+1):
        ...
        dataset = TaskDataset(cfg.data_file["train"], pipelines[:i], n_data=16)
        print('\n', dataset[0], '\n', dataset[1], '\n')
