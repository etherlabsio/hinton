...
### hyperparameters for training: can be tweaked from the json file
...
class TrainLoop(object):
    ...
    def train(self, get_loss, model_file=None, pretrain_file=None, data_parallel=False):
        """ Train Loop """
    ...
    def nsp_scorer(self, evaluate, model_file):
        """ Evaluation Loop """
        self.model.eval() # evaluation mode
        self.load(model_file, None)
        model = self.model.to(self.device)
        ...
        for batch in self.data_iter:      
            batch = [t.to(self.device) for t in batch]
            with torch.no_grad(): # evaluation without gradient calculation
                accuracy, result, nsp_score = evaluate(model, batch) # accuracy to print
            results.append(nsp_score)
        return results[0][0]

    def feature_extraction(self, evaluate, model_file):
        """ Evaluation Loop """
        self.model.eval() # evaluation mode
        self.load(model_file, None)
        model = self.model.to(self.device)
        ...
        for batch in self.data_iter:      
            batch = [t.to(self.device) for t in batch]
            with torch.no_grad(): # evaluation without gradient calculation
                vector = evaluate(model, batch) # accuracy to print
        ...
        if model_file:
            ...
            if pretrain_file.endswith('.ckpt'): # checkpoint file in tensorflow
                ...
                self.model.transformer.load_state_dict(
                    {key[12:]: value
                        for key, value in torch.load(pretrain_file,map_location='cpu').items()
