...
def train(args, train_dataset, model, tokenizer, labels, pad_token_label_id):
    ...
    for ep_num in train_iterator:
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
                        results, _ = evaluate(args, model, tokenizer, labels, pad_token_label_id,mode="dev",prefix=global_step)
        ...
        if args.local_rank in [-1, 0] and args.save_steps > 0:
            ...
            output_dir = os.path.join(args.output_dir, "epoch-{}".format(ep_num))
...
def evaluate(args, model, tokenizer, labels, pad_token_label_id, mode, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            print(tmp_eval_loss)
            if args.n_gpu > 1:
                tmp_eval_loss = tmp_eval_loss.mean()  # mean() to average on multi-gpu parallel training
